...
class ApiTest(unittest.TestCase):
    ...
    def test_check_status_MalformedRequestException(self):
        ...
        self.assertRaises(MalformedRequestException, self.api.check_status, self.response.content, self.response)

    def test_check_status_UnAuthorisedException(self):
        self.response.status_code = 401
        self.assertRaises(UnAuthorisedException, self.api.check_status, self.response.content, self.response)

    def test_check_status_NotFoundException(self):
        self.response.status_code = 404
        self.assertRaises(NotFoundException, self.api.check_status, self.response.content, self.response)

    def test_check_status_UnAcceptableContentException(self):
        self.response.status_code = 406
        self.assertRaises(UnAcceptableContentException, self.api.check_status, self.response.content, self.response)

    def test_check_status_InvalidRequestException(self):
        self.response.status_code = 422
        self.assertRaises(InvalidRequestException, self.api.check_status, self.response.content, self.response)

    def test_check_status_RateLimitException(self):
        self.response.status_code = 429
        self.assertRaises(RateLimitException, self.api.check_status, self.response.content, self.response)

    def test_check_status_ServerException_1(self):
        self.response.status_code = 500
        self.assertRaises(ServerException, self.api.check_status, self.response.content, self.response)
    ...
    def test_make_request_1(self):
        response = self.api.make_request(url_parameter=self.url_parameter, body=self.request_data)
